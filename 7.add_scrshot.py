from tkinter import *
import tkinter.ttk as ttk    # 콤보박스, 프로그레스 바
...
try : 
    ...
    photo = PhotoImage(file="gui_project/banner.png")
    img_label = Label(banner_frame, image=photo, height=70)
    img_label.pack()


    # 엔트리 프레임
    entry_frame = LabelFrame(root, text="Capture", relief="solid", bd=1)
    entry_frame.pack(fill='x', padx=5, pady=5)

    entry = Entry(entry_frame, width=80)
    entry.insert(0, "Type the key name, eg. F9.    Push the key whenever you want to capture till you push ESC !")
    entry.pack(side="left", padx=5, pady=5, ipady=5)
    ...
    def capture() :
        ...
        keyboard.add_hotkey(entry.get(), scrshot)   # F9 키 실행값 등록
        keyboard.wait("Esc")   

    cap_btn = Button(entry_frame, text="Start", width=7, command=capture)
    cap_btn.pack(side="right", padx=5, pady=5, ipady=5)
...
list_file = Listbox(list_frame, selectmode="extended", height=8, yscrollcommand=scrollbar.set)
...
cmb_width = ttk.Combobox(option_frame, state="readonly", values=opt_width, width=15)
cmb_width.current(0)        # index: 0 값을 자동으로 세팅
cmb_width.pack(side="left", padx=5, pady=5)

# 2. 간격 옵션
lbl_gap = Label(option_frame, text="Gap", width=5)
lbl_gap.pack(side="left", padx=[50,5], pady=5)
# 콤보박스
opt_gap = ["None", "Narrow", "Normal", "Wide"]
cmb_gap = ttk.Combobox(option_frame, state="readonly", values=opt_gap, width=10)
cmb_gap.current(0)
cmb_gap.pack(side="left", padx=5, pady=5)

# 3. 파일 포맷 옵션
lbl_form = Label(option_frame, text="Format", width=6)
lbl_form.pack(side="left", padx=[50,5], pady=5)
# 콤보박스
opt_form = ["JPG", "PNG", "BMP"]
cmb_form = ttk.Combobox(option_frame, state="readonly", values=opt_form, width=10)
cmb_form.current(0)
